...
class DiffusionReaction2DDataset(Dataset):
    ...
    def _prepare(self):
        ...
        if self.mesh_query_type == 'random_custom':
            ...
            v_query_sampled = v_query_sampled[:, :, ::self.reduced_resolution_t, :]
            ...
            v_query_sampled = v_query_sampled[sample_query_index_list, ...]
        ...
        self.label_y['v'].append(v_query_sampled)
        self.label_y['uv_mean'].append(self.uv_mean)
        self.label_y['uv_std'].append(self.uv_std)
...
class LoadDiffusionReaction2DDatasetDGL(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_params):
        ...
        if 'train' in dataset_params['dataset_split']:
            self.train = DiffusionReaction2DDataset(dataset_params, split='train')
        if 'val' in dataset_params['dataset_split']:
            self.val = DiffusionReaction2DDataset(dataset_params, split='val')
        if 'test' in dataset_params['dataset_split']:
            self.test = DiffusionReaction2DDataset(dataset_params, split='test')
