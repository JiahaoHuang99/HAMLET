...
if __name__ == '__main__':
    ...
    # AYL
    data_source_path = '/media/ssd/data_temp/PDE/data/DiffusionReaction/PDEBench'
    data_file_path_raw = os.path.join(data_source_path, 'raw/2D/diffusion-reaction/2D_diff-react_NA_NA.h5')
    ...
    # # ---------------------------
    # # FIXME: DEBUG
    # save_dir = '/home/jh/physics_graph_transformer/physics_graph_transformer/tmp/hist/dr'
    # mkdir(save_dir)
    #
    # u_list = []
    # for i in range(n):
    ...
    # t = u_all.shape[1]
    # for i in range(t):
    #     u_t = u_all[:, i, :, 0, :].reshape(-1)
    #     # plt.imshow(u_t)
    #     plt.hist(u_t)
    #     mkdir(os.path.join(save_dir, 'u'))
    #     plt.savefig(os.path.join(save_dir, 'u', 'u_{:03d}.png'.format(i)))
    #     plt.close()
    #
    #     v_t = u_all[i, :, :, 1, :].reshape(-1)
    #     # plt.imshow(v_t)
    #     plt.hist(v_t)
    #     mkdir(os.path.join(save_dir, 'v'))
    #     plt.savefig(os.path.join(save_dir, 'v', 'v_{:03d}.png'.format(i)))
    #     plt.close()
